...
{
    ros::init(argc, argv, "camera4_node");
    ros::NodeHandle nh;
    ...
    image_pub = it.advertise("/camera4/image", 1);
    ...
    TcamImage cam("18710033");//"17710016");//"27710206");
    //TcamCamera cam("00001234");
    // Set a color video format, resolution and frame rate
    cam.set_capture_format("BGRx",FrameSize{2048,1536}, FrameRate{30,1});
    //FrameSize{640,480}, FrameRate{366,1});// FrameSize{2048,1536}, FrameRate{36,1});
    ...
    while(nh.ok()){    
        ...
        std::cout<<"frame #:"<<num_of_frames<<std::endl;
        ros::spinOnce();
        loop_rate.sleep();
    }
